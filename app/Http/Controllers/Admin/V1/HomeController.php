...
class HomeController extends Controller
{
    ...
    {
        $result = DB::table('users')->whereBetween('created_at', [now()->addDays(-7)->startOfDay(), now()->endOfDay()])
            ->selectRaw('date(created_at) as date,count(*) as num')->groupBy('date')->pluck('num', 'date')->toArray();
        for ($i = 7; $i >= 1; $i--) {
            if (!array_key_exists(now()->subDays($i)->toDateString(), $result)) {
                $result[now()->subDays($i)->toDateString()] = 0;
            }
        }
        ksort($result);
        $data = [
